...
describe('Pair', () => {
  const USDC = new Token(ChainId.GÖRLI, '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', 18, 'USDC', 'USD Coin')
  const DAI = new Token(ChainId.GÖRLI, '0x6B175474E89094C44Da98b954EedeAC495271d0F', 18, 'DAI', 'DAI Stablecoin')
  ...
  describe('#priceOf', () => {
    ...
    it('throws if invalid token', () => {
      expect(() => pair.priceOf(WETH[ChainId.GÖRLI])).toThrow('TOKEN')
    })
  })
  ...
  describe('#reserveOf', () => {
    ...
    it('throws if not in the pair', () => {
      expect(() =>
        new Pair(new TokenAmount(DAI, '101'), new TokenAmount(USDC, '100')).reserveOf(WETH[ChainId.GÖRLI])
      ).toThrow('TOKEN')
    })
  })
  ...
  describe('#chainId', () => {
    it('returns the token0 chainId', () => {
      expect(new Pair(new TokenAmount(USDC, '100'), new TokenAmount(DAI, '100')).chainId).toEqual(ChainId.GÖRLI)
      expect(new Pair(new TokenAmount(DAI, '100'), new TokenAmount(USDC, '100')).chainId).toEqual(ChainId.GÖRLI)
    })
  })
  describe('#involvesToken', () => {
    ...
    expect(
      new Pair(new TokenAmount(USDC, '100'), new TokenAmount(DAI, '100')).involvesToken(WETH[ChainId.GÖRLI])
    ).toEqual(false)
  })
})
